	...
	for dirn in dirnames: 									#All the words in sign language
		if dirn.lower() not in classDict:
			...
			for dirname3 in dirs: 								#list of all the people who signed that word
				...
				meanUpper = np.mean(upper['original'],axis=0)
				meanLower = np.mean(lower['original'],axis=0)

				for i in xrange(len(upper['original'])):
					upper['original'][i] = upper['original'][i] - meanUpper
					lower['original'][i] = lower['original'][i] - meanLower

					upImage = upper['original'][i]
					lowImage = lower['original'][i]
...
writePickle(classDict,"classDict.pickle")
